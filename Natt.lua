...
function Nautilus:__init()
    self.QPrediction = GGPrediction:SpellPrediction ({Type = _G.SPELLTYPE_LINE, Delay = 0.25, Radius = 90, Range = 1100, Speed = 2000, Collision = true})
...
function Nautilus:LoadMenu()
    ...
    self.tyMenu:MenuElement({type = MENU, id = "Combo", name = "Combo"})
        ...
        self.tyMenu.Combo:MenuElement({id = "range", name = "Max Cast Q In range", value = 1000, min = 1, max = 1100, step = 1})
...
function Nautilus:CastQ(target)
    if Ready(_Q) and lastQ +350 < GetTickCount() and orbwalker:CanMove() then
        local Pred = GetPrediction(target, self.QPrediction, myHero)
...
function Nautilus:Combo()
    ...
    target = self:GetTarget(targetList, self.QPrediction.Range)
